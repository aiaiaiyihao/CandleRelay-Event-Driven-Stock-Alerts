      ...
      {page === 'dashboard' && <section className="dashboard-section page-surface" id="dashboard">
        ...
        {chatOpen && <section className="market-chat panel" role="dialog" aria-label="Market Assistant">
          ...
          <div className="market-chat-body">
            ...
            <div className="market-chat-prompts">{['How is the market?', 'Which stocks are strongest today?', 'Why are the weakest stocks falling?'].map((prompt) => <button key={prompt} onClick={() => askMarket(prompt)} disabled={busy === 'market-chat'}>{prompt}</button>)}</div>
