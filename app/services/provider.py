...
from app.services.iex_service import fetch_price_iex
from app.services.alpha_vantage_service import fetch_price_alpha
...
async def fetch_price_by_provider(provider: str, symbol: str, poll: bool):
    ...
    if cached:
        ...
        logging.info(
            f"🗄️  Cache hit: {symbol} | price={data['price']} | ts={data['timestamp']} (provider={provider})"
        )
        return data

    # 2️⃣ Fetch from external provider
    try:
        if provider == "yfinance":
            data = await fetch_price_yfinance(symbol)
        elif provider == "iex":
            data = await fetch_price_iex(symbol)
        elif provider == "alpha_vantage":
            data = await fetch_price_alpha(symbol)
        else:
